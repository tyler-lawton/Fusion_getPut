...
try:
    ...
    def traverseAndReplace(obj, objName, varMap=None, path=None):
        ...
        if isinstance(varMap, dict):
            ...
            if isinstance(obj, dict):
                value = {k: traverseAndReplace(v, objName, varMap, path + [k]) for k, v in obj.items()}
            elif isinstance(obj, list):
                value = [traverseAndReplace(elem, objName, varMap, path + [[]]) for elem in obj]
            else:
    ...
    def doPostByIdThenPut(
        apiUrl,
        payload,
        type,
        putParams="?_cookie=false",
        postParams="?_cookie=false",
        idField="id",
        usr=None,
        pswd=None,
        existsChecker=None,
        ...
        if existsChecker(response, payload):
            ...
            if f"{id} not in app" in response.text or re.search(
                f"The (Task|Collection|Data Model) with id '{id}' does not exist", response.text
            ):
                ...
                if lresponse.status_code >= 200 and lresponse.status_code <= 250:
                    ...
                    response = requests.put(
                        url, auth=auth, headers=headers, data=json.dumps(payload), verify=isVerify()
                    )
        ...
        if lresponse and lresponse.status_code < 200 or lresponse.status_code > 250:
            eprint(
                "Non OK response: {}   when linking object {} to App {}".format(
                    str(lresponse.status_code), payload["subject"], appName
                )
            )
    ...
    if __name__ == "__main__":
        ...
        parser.add_argument(
            "-d", "--dir", help="Input directory (with a *_APP.json file), required.", required=True
        )  # ,default="default"
        parser.add_argument(
            "--failOnStdError",
        ...
        parser.add_argument(
            "--password", help="Fusion password,  default: ${lw_PASSWORD} or 'password123'."
        )  # ,default="password123"
        parser.add_argument("--jwt", help="JWT token for authentication.  If set, --password is ignored", default=None)

        parser.add_argument(
            "--debug", help="Print debug messages while running, default: False.", default=False, action="store_true"
        )  # default=False
        parser.add_argument(
            "--noVerify",
        ...
        parser.add_argument(
            "-v", "--verbose", help="Print details, default: False.", default=False, action="store_true"
        )  # default=False
        parser.add_argument(
            "--varFile",
